...
class ImageComponent extends Component
{
    ...
    public function propDimensions($w, $h)
    {
    ...
    public function propDimensionsHid($w, $h)
    {
        if ($this->image_width != 0) {
            $cw = $this->image_width;
            $ch = $this->image_height;
    ...
    public function saveTo($location)
    {
        if (!$this->width && !$this->height) {
            ...
            if ($this->width > $this->image_width && $this->height > $this->image_height) {
                $this->height = $this->image_height;
                $this->width = $this->image_width;
            } else if ($this->width / $this->height <= $this->image_width / $this->image_height) {
                $height = round($this->width / ($this->image_width / $this->image_height));
                $this->height = $height;
                /*                if($height!=$this->height) {
                                    $percentage = ($this->image_height*100)/$height;
